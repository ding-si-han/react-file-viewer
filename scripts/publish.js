...
inquirer
	.prompt([
		{
			...
			name: 'publish',
			message:
				'Select a version to publish. This will run all tests and build first.',
			default: 0,
	...
	.then(function (answers) {
		...
		if (branchString !== 'master') {
			console.log(
				errColor(
					`must be on master branch to publish. Currently on '${branchString}'`
				)
			);
			console.log(errColor('Aborting publish'));
			return;
		...
		try {
			...
			execSync('npm publish', execOpts, ' --access public');
		} catch (e) {
